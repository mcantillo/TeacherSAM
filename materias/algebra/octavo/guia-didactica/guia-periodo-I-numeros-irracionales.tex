...
\begin{tikzpicture}[scale=1.3, every node/.style={font=\small}]
  ...
  \node[fill=white, inner sep=1pt] at ($(O)!0.72!(P2)$) {$\sqrt{2}$};
  \node[fill=white, inner sep=1pt] at ($(O)!0.76!(P3)$) {$\sqrt{3}$};
  \node[fill=white, inner sep=1pt] at ($(O)!0.78!(P4)$) {$2$};
  \node[fill=white, inner sep=1pt] at ($(O)!0.8!(P5)$) {$\sqrt{5}$};
  \node[fill=white, inner sep=1pt] at ($(O)!0.8!(P6)$) {$\sqrt{6}$};
\end{tikzpicture}
\end{center}
...
cada kilómetro, un error del \qty{0,04}{\percent}.
\end{aplicacion}
