...
- Design constraints the teacher asked for: **printed in black & white** (white page, ink + grays only; plot curves distinguished by dash style via the `mmc` pgfplots cycle list, so use `\addplot+`), logo at full opacity, small, top-left. Header = school name, área, year: «Área de Matemáticas» by default, and «Área de Ciencias Naturales» automatically when `\asignatura{Física}` (teacher's decision, 2026-09-15); `\area{…}` after `\asignatura` overrides it. Footer = teacher name · Docente · school + page X / Y.
